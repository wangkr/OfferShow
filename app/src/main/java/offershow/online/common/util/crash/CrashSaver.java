package offershow.online.common.util.crash;
...
import offershow.online.common.util.string.MD5;

class CrashSaver {

	public static final int REQUEST_WRITE_STORAGE = 123;

	public static final void save(final Context context, Throwable ex,
			boolean uncaught) {
		...
		if (OfferShowApp.isAndroid6() && ContextCompat.checkSelfPermission(context, Manifest.permission.WRITE_EXTERNAL_STORAGE) != PackageManager.PERMISSION_GRANTED) {
			ActivityCompat.requestPermissions((Activity)context, new String[]{Manifest.permission.READ_EXTERNAL_STORAGE}, REQUEST_WRITE_STORAGE);
