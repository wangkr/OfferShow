package offershow.online.common.util.media;
...
import offershow.online.common.util.file.AttachmentStore;
import offershow.online.common.util.file.FileUtil;
import offershow.online.common.util.log.LogUtil;
import offershow.online.common.util.storage.StorageType;
import offershow.online.common.util.storage.StorageUtil;
import offershow.online.common.util.string.StringUtil;
...
public class ImageUtil {
    ...
    public static int[] getBoundWithLength(int maxSide, Object imageObject, boolean resizeToDefault) {
        ...
        if (String.class.isInstance(imageObject)) {
        	...
        	bound = BitmapDecoder.decodeBound(OfferShowApp.getContext().getResources(), (Integer) imageObject);
        	width = bound[0];
        	height = bound[1];
    ...
    public static Bitmap getBitmapFromDrawableRes(int res) {
        try {
            return getBitmapImmutableCopy(OfferShowApp.getContext().getResources(), res);
        } catch (Exception e) {
            e.printStackTrace();
            return null;
        }
    ...
    public static Bitmap getScreenFitBitmap(File srcFile) {
        LogUtil.d("ImageUtil", "srcFile size="+srcFile.length());
        BitmapFactory.Options options = new BitmapFactory.Options();
        Bitmap dstBitmap = null, srcBitmap = null;
        ...
        float scnRatio = ScreenUtil.screenWidth / ScreenUtil.screenHeight;
        if (ratio >= scnRatio) {
            // 原图宽高比大于屏幕宽高比
            dstWidth = ScreenUtil.screenWidth;
            dstHeight = (int)(dstWidth/ratio);
        } else {
            dstHeight = ScreenUtil.screenHeight;
            dstWidth = (int)(dstHeight*ratio);
        }
        float scale = (float)dstWidth / width;
        try {
            int inSampleSize = SampleSizeUtil.calculateSampleSize(srcFile.getPath(), dstWidth * dstHeight);
            srcBitmap = BitmapDecoder.decodeSampled(srcFile.getPath(), inSampleSize);
            // 旋转
            ExifInterface localExifInterface = new ExifInterface(srcFile.getAbsolutePath());
            int rotateInt = localExifInterface.getAttributeInt(ExifInterface.TAG_ORIENTATION, ExifInterface.ORIENTATION_NORMAL);
            float rotate = getImageRotate(rotateInt);
            if (rotate == 1.0f && scale <= 1.0) {
                return srcBitmap;
            } else {
                try {
                    Matrix matrix = new Matrix();
